...
public class EntranceFrame extends JFrame{
	...
	public EntranceFrame(){
		super();
		this.setTitle("Cartel-ControleEntree");
		...
		//ICONE
		try {
			Image img = ImageIO.read(ClassLoader.getSystemResourceAsStream("img/icon.png"));
	...
	public void initMainPanel(){
		//FOND
		try {
			mainPanel=new BackgroundPanel(new File(ClassLoader.getSystemResource("img/EntrancePanel.png").toURI()));
		...
		cliNomLabel.setBounds(0, 75, 940, 50);
		...
		cliPrenomLabel.setBounds(0, 135, 940, 50);
		...
		cliDelegationLabel.setBounds(0, 255, 940, 50);
		...
		cliAuthLabel.setBounds(0, 380, 940, 50);
	...
	private void updateAuthPanel(int i){
		...
		if(authorized==1){
			...
			cliAuthLabel.setText("D\u00c9J\u00c0 ENTR\u00c9(E)");
		}
	}
	
	private void initFonts(){
		try {
			light=Font.createFont(Font.TRUETYPE_FONT,ClassLoader.getSystemResourceAsStream("fonts/TextaNarrow-Light.ttf"));
			medium=Font.createFont(Font.TRUETYPE_FONT,ClassLoader.getSystemResourceAsStream("fonts/TextaNarrow-Medium.ttf"));
		...
		SwingWorker<Client,Integer> readWorker = new SwingWorker<Client, Integer>(){
			...
			protected Client doInBackground() throws Exception {
				try {
					...
					//LECTURE AUTORISATIONS SOIREES
					String auths=read(9);
					
					//ECRITURE DU FAIT QUE LA PERSONNE SOIT ENTREE
